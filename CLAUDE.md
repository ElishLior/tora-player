...
- Public catalog reads: `src/lib/supabase/anon.ts` has a cookie-less anon client (`createAnonSupabaseClient`, published content only via RLS) and `unstable_cache` readers (`getCached*`, tag `catalog`, 5 min), plus `createCatalogLessonListReader()` for `/lessons`, `/tags/[tag]` and `/search`. Use them for public data in pages. Never use them for per-user or admin (unpublished) data. Every action that writes catalog data calls `revalidateCatalog()` to invalidate the tag, public layout, feeds and sitemap; writes outside `src/actions` (upload API routes, scripts) show up after their independent revalidation intervals.
...
- Podcast RSS: `/feed.xml` (all published non-short lessons, paginated) and `/series/<id>/feed.xml` (published series lessons, including legacy single-file audio) are built by `src/lib/podcast-feed.ts`: one episode per audio file, stable GUID = `lesson_audio.id` or the lesson id for legacy audio. Enclosure = absolute `/api/audio/download/<key>?disposition=inline`; GET redirects to a signed R2 URL, while HEAD reads R2 object metadata and returns matching representation headers without audio bytes. Artwork `public/brand/podcast-cover.jpg` (1400², from `scripts/generate-icons.mjs`). Most audio is Opus (`audio/ogg`), which Apple Podcasts does not play; Pocket Casts/AntennaPod/Overcast do.
- `/feed.xml` and `src/app/sitemap.ts` are dynamic route handlers: CI builds without database access never ship an empty/listing-only prerender. Public data is cached on first runtime request for 15 minutes and 1 hour respectively via `unstable_cache` with the `catalog` tag; `revalidateCatalog()` invalidates both after admin writes. A failed cold read returns an error rather than a misleading empty feed or sitemap.
...
- Sleep timer (`sleepTimer` in the audio store, not persisted; UI `src/components/player/sleep-timer.tsx`): minutes use a wall-clock timeout with 10s fade-out while playing and a deadline check on status/part boundaries, even during buffering. End-of-part pauses at that file's end and the next Play advances; end-of-lesson pauses on its last part (or last available file of an older offline queue with unknown total).
...
- Offline lesson cache uses IndexedDB (`src/lib/offline-storage.ts`); each saved file is a Blob played through a blob URL, with the source lesson's part index and total stored alongside it. A subset must never invent a smaller total.
- Offline save and local file download both use `/api/audio/download/[fileKey]`: GET 302-redirects to a short-lived presigned R2 URL (audio bytes never pass through Vercel), while HEAD reads object metadata via R2 and returns the same byte length, MIME and disposition GET would serve. `?filename=` gives an attachment with an RFC 5987 UTF-8 filename; `?disposition=inline` is raw audio for offline saving and RSS enclosures. Only `audio/…` keys with an audio extension are accessible.
